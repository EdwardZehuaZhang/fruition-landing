        ...
        {statement && (
          <p
            ...
            style={{
              ...
              color: dark ? "var(--white)" : "var(--ink-heading)",
            }}
          >
            {statement}
          </p>
          ...
          <div className="mt-8 flex flex-wrap items-start justify-center gap-x-8 gap-y-6 md:gap-12">
            {metrics.map((m, i) => (
              <div key={m._key || i} className="flex flex-col items-center" style={{ minWidth: 140 }}>
                <span className="font-bold" style={{ fontSize: 40, lineHeight: "44px", color: dark ? "var(--white)" : "var(--brand)" }}>
                  {m.value}
                </span>
                <span style={{ fontSize: 14, lineHeight: "20px", color: dark ? "rgba(255,255,255,0.72)" : "var(--ink-muted)", marginTop: 6 }}>
                  {m.text}
                </span>
              </div>
            ))}
