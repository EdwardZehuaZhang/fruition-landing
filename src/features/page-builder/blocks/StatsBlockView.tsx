...
function renderHeadingWithAccent(heading: string, accent?: string) {
  ...
  if (idx >= 0) {
    return (
      <>
        <span className="text-ink">{heading.slice(0, idx)}</span>
        <span className="text-brand">{target}</span>
        <span className="text-ink">{heading.slice(idx + target.length)}</span>
      </>
    )
  }
  return <span className="text-ink">{heading}</span>
}
      ...
      <div className="mx-auto max-w-[1040px] flex flex-col items-center gap-8">
        ...
        {subheading && (
          showMondayPartnersBadge ? (
            <div className="flex items-start gap-[5px] text-[14px] text-ink">
              <span>{subheading}</span>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={mondayPartnersBadgeSrc} alt="monday.com partners" width={148} height={23} className="h-[23px] w-auto" />
            </div>
          ) : (
            <p className="text-base sm:text-lg text-center text-ink-soft max-w-[600px]">{subheading}</p>
          )
        )}
          ...
          <div className="flex flex-wrap justify-center gap-8 sm:gap-12 lg:gap-16 mt-4">
            {stats.map((stat, i) => (
              <div key={stat._key ?? i} className="flex flex-col items-center gap-1 min-w-[120px]">
                ...
                <span className="text-3xl sm:text-4xl lg:text-[44px] font-bold text-brand leading-tight whitespace-nowrap">
                  {stat.value}
                </span>
                <span className="text-sm sm:text-[15px] text-ink-soft leading-snug text-center mt-1">{stat.label}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {ctaLabel && ctaUrl && (
          <Link
            href={ctaUrl}
            className="inline-flex items-center justify-center gap-2 px-10 py-4 rounded-full bg-gradient-to-r from-[var(--purple-primary)] to-[var(--purple-light)] hover:bg-[var(--accent-blue)] hover:bg-none text-white text-lg sm:text-xl font-bold tracking-wide hover:scale-[1.02] active:scale-[0.98] transition-all shadow-lg shadow-purple-500/20 mt-2"
          >
