  ...
  return (
    <section className="bg-surface px-4" style={{ paddingTop: 32, paddingBottom: 32 }}>
      <div className="mx-auto text-center" style={{ maxWidth: 920 }}>
        <p style={{ color: "var(--ink-soft)", fontSize: 16, lineHeight: "26px" }}>
          With monday.com <span className="font-bold" style={{ color: "var(--brand)" }}>CRM</span> and{" "}
          <span className="font-bold" style={{ color: "var(--brand)" }}>Work Management</span> as your Construction software, your teams will experience simplified and streamlined communication with mobile access and improved automated workflow efficiency.
        </p>
      </div>
    </section>
  )
  ...
  return (
    <section className="bg-surface px-4" style={{ paddingTop: 80, paddingBottom: 80 }}>
      <div className="mx-auto" style={{ maxWidth: 1100 }}>
        <h2 className="text-center font-bold" style={{ color: "var(--ink-heading)", fontSize: 32, lineHeight: "42px", marginBottom: 40 }}>
          Support Each Stage of Your Project Life Cycle with a <span style={{ color: "var(--brand)" }}>monday.com Expert</span>
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: 28 }}>
          {stages.map((s, i) => (
            <div key={s.n || i} className="flex flex-col" style={{ gap: 10, padding: 24, borderRadius: 16, border: "1px solid var(--line-tint)", background: "var(--surface)", boxShadow: "0 12px 28px -22px rgba(64,12,140,0.18)" }}>
              <p style={{ color: "var(--brand)", fontSize: 36, fontWeight: 300, lineHeight: 1 }}>{s.n}</p>
              <p className="font-bold" style={{ color: "var(--ink-heading)", fontSize: 18 }}>{s.title}</p>
              <p style={{ color: "var(--ink-soft)", fontSize: 14, lineHeight: "22px" }}>{s.body}</p>
            </div>
          ))}
        </div>
      </div>
...
function ConstructionTestimonialsSection({ testimonials }: { testimonials: ConstructionTestimonial[] }) {
  return (
    <section className="px-4" style={{ paddingTop: 80, paddingBottom: 80, background: "linear-gradient(180deg, #faf6ff 0%, #ebd9ff 100%)" }}>
      <div className="mx-auto" style={{ maxWidth: 1100 }}>
        <h2 className="text-center font-bold" style={{ color: "var(--ink-heading)", fontSize: 32, lineHeight: "42px", marginBottom: 40 }}>
          Construction <span style={{ color: "var(--brand)" }}>Testimonials</span>
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: 24 }}>
          {testimonials.map((t, i) => (
            <figure key={t.title || i} className="bg-surface overflow-hidden flex flex-col" style={{ borderRadius: 20, border: "1px solid rgba(128,21,232,0.08)", boxShadow: "0 18px 36px -24px rgba(64,12,140,0.22)" }}>
              <div style={{ aspectRatio: "16 / 10", overflow: "hidden", background: "var(--surface-tint-2)" }}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={t.image} alt={t.title} className="w-full h-full object-cover" />
              </div>
              <div style={{ padding: 28 }}>
                <p className="font-bold" style={{ color: "var(--brand)", fontSize: 13, letterSpacing: "0.14em", textTransform: "uppercase", marginBottom: 14 }}>{t.title}</p>
                <blockquote style={{ color: "var(--ink)", fontSize: 15, lineHeight: "24px" }}>“{t.quote}”</blockquote>
    ...
    <section className="bg-surface px-4" style={{ paddingTop: 80, paddingBottom: 80 }}>
      <div className="mx-auto" style={{ maxWidth: 1100 }}>
            ...
            <button
              ...
              style={{
                ...
                ...(i === activeIdx
                  ...
                  : { background: "var(--surface)", color: "var(--ink-heading)", border: "1px solid var(--line)" }),
              }}
            >
              {tab.label}
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: 20 }}>
          {active.items?.map((it) => (
            <div key={it._key} className="bg-surface" style={{ padding: 24, borderRadius: 18, border: "1px solid rgba(128,21,232,0.08)", boxShadow: "0 12px 28px -22px rgba(64,12,140,0.18)", display: "flex", flexDirection: "column", gap: 12 }}>
              <div className="flex items-center" style={{ gap: 14 }}>
                ...
                <p className="font-bold" style={{ color: "var(--ink-heading)", fontSize: 15, lineHeight: "22px" }}>{it.title}</p>
              </div>
              <p style={{ color: "var(--ink-soft)", fontSize: 13, lineHeight: "20px", whiteSpace: "pre-line" }}>{it.description}</p>
            </div>
          ))}
        </div>
      </div>
