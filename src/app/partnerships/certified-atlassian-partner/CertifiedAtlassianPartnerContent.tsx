    ...
    <section className="bg-surface px-4" style={{ paddingTop: 80, paddingBottom: 80 }}>
      <div className="mx-auto" style={{ maxWidth: 1100 }}>
        <h2 className="text-center font-bold" style={{ color: "var(--ink-heading)", fontSize: 36, lineHeight: "44px", marginBottom: 40 }}>
          Proven <span style={{ color: "var(--brand)" }}>Atlassian Cloud</span> Results
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3" style={{ gap: 20 }}>
          {stats.map((s: ProvenStat, i: number) => (
            <div key={s._key || s.value || i} className="text-center" style={{ padding: 32, borderRadius: 18, background: "linear-gradient(180deg, var(--surface-tint) 0%, var(--line-tint) 100%)", border: "1px solid rgba(128,21,232,0.10)", boxShadow: "0 12px 28px -22px rgba(64,12,140,0.18)", display: "flex", flexDirection: "column", alignItems: "center", gap: 14 }}>
              <span className="flex items-center justify-center" style={{ width: 56, height: 56, borderRadius: 999, background: "var(--surface)", fontSize: 26, boxShadow: "0 4px 14px -8px rgba(64,12,140,0.25)" }}>{s.emoji}</span>
              <p className="font-bold" style={{ color: "var(--brand)", fontSize: 32, lineHeight: 1 }}>{s.value}</p>
              <p style={{ color: "var(--ink-soft)", fontSize: 13, lineHeight: "20px", maxWidth: 240 }}>{s.body}</p>
            </div>
          ))}
        </div>
      </div>
...
function ComprehensiveServicesSection({ cards }: { cards: ServiceCard[] }) {
  ...
  return (
    <section className="bg-surface px-4" style={{ paddingTop: 80, paddingBottom: 80 }}>
      <div className="mx-auto" style={{ maxWidth: 1100 }}>
        <h2 className="text-center font-bold" style={{ color: "var(--ink-heading)", fontSize: 36, lineHeight: "44px", marginBottom: 40 }}>
          Our Comprehensive <span style={{ color: "var(--brand)" }}>Atlassian</span> Services
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3" style={{ gap: 20 }}>
          {cards.map((c: ServiceCard, i: number) => (
            <div key={c._key || c.title || i} className="bg-surface" style={{ padding: 24, borderRadius: 16, border: "1px solid var(--line-tint)", boxShadow: "0 12px 28px -22px rgba(64,12,140,0.18)", display: "flex", flexDirection: "column", gap: 12 }}>
              <span style={{ fontSize: 28 }}>{c.emoji}</span>
              <p className="font-bold" style={{ color: "var(--ink-heading)", fontSize: 16 }}>{c.title}</p>
              <p style={{ color: "var(--ink-soft)", fontSize: 13, lineHeight: "20px" }}>{c.body}</p>
              {c.bullets && c.bullets.length > 0 && (
                <ul className="flex flex-col" style={{ gap: 6, marginTop: 4 }}>
                  {c.bullets.map((b: string, j: number) => (
                    <li key={b || j} className="flex items-start" style={{ gap: 8, color: "var(--ink-soft)", fontSize: 12, lineHeight: "18px" }}>
    ...
    <section className="px-4" style={{ paddingTop: 80, paddingBottom: 80, background: "linear-gradient(180deg, var(--surface-tint) 0%, var(--line-tint) 100%)" }}>
      <div className="mx-auto" style={{ maxWidth: 1100 }}>
        <div className="flex flex-col items-center text-center" style={{ marginBottom: 56 }}>
          <h2 className="font-bold" style={{ color: "var(--ink-heading)", fontSize: 36, lineHeight: "44px", marginBottom: 22 }}>
            Your <span style={{ color: "var(--brand)" }}>Atlassian</span> Experts
          </h2>
          <Link
            href={calendlyUrl}
          ...
          {cards.map((c: ExpertCard, i: number) => (
            ...
            >
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ color: "var(--ink-soft)", fontSize: 15, lineHeight: "25px" }}>{c.body}</p>
              </div>
              <div
