  ...
  return (
    <section className="bg-surface px-4" style={{ paddingTop: 60, paddingBottom: 60 }}>
      <div className="mx-auto text-center" style={{ maxWidth: 880 }}>
        ...
        <p style={{ fontSize: 16, lineHeight: "26px", color: "var(--ink-soft)", marginTop: 14 }}>
    ...
    <section className="bg-surface px-4" style={{ paddingTop: 80, paddingBottom: 80 }}>
      <div className="mx-auto" style={{ maxWidth: 1100 }}>
        <div className="text-center" style={{ marginBottom: 40 }}>
          <h2 className="font-bold" style={{ color: "var(--ink-heading)", fontSize: 36, lineHeight: "44px", marginBottom: 14 }}>
            Transform Your Business with ClickUp&apos;s <span style={{ color: "var(--brand)" }}>Everything App for Work</span>
          </h2>
          <p className="mx-auto" style={{ color: "var(--ink-soft)", fontSize: 16, lineHeight: "26px", maxWidth: 820 }}>
          ...
          <p className="mx-auto" style={{ color: "var(--ink-soft)", fontSize: 16, lineHeight: "26px", maxWidth: 820, marginTop: 12 }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: 20 }}>
          {cards.map((c, i) => (
            <div key={c.title || i} className="bg-surface" style={{ padding: 24, borderRadius: 18, border: "1px solid var(--line-tint)", boxShadow: "0 12px 28px -22px rgba(64,12,140,0.18)", display: "flex", flexDirection: "column", gap: 10 }}>
              <span style={{ fontSize: 28 }}>{c.emoji}</span>
              <p className="font-bold" style={{ color: "var(--ink-heading)", fontSize: 18 }}>{c.title}</p>
              <p style={{ color: "var(--ink-soft)", fontSize: 14, lineHeight: "22px" }}>{c.body}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mx-auto" style={{ maxWidth: 1100 }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: 20 }}>
          {(active.groups || []).map((g: FeatureGroup, gi: number) => (
            <div key={g.number || gi} className="bg-surface" style={{ padding: 24, borderRadius: 18, border: "1px solid rgba(128,21,232,0.08)", boxShadow: "0 12px 28px -22px rgba(64,12,140,0.18)", display: "flex", flexDirection: "column", gap: 14 }}>
...
function IndustryTabsSection({ tabs }: { tabs: IndustryTab[] }) {
  ...
  if (!active) return null
  return (
    <section className="px-4" style={{ paddingTop: 80, paddingBottom: 80, background: "linear-gradient(180deg, var(--surface-tint) 0%, var(--line-tint) 100%)" }}>
      <div className="mx-auto" style={{ maxWidth: 1100 }}>
        <h2 className="text-center font-bold" style={{ color: "var(--ink-heading)", fontSize: 36, lineHeight: "44px", marginBottom: 32 }}>
          Implement ClickUp for <span style={{ color: "var(--brand)" }}>any team</span>
        </h2>
          ...
          {tabs.map((tab, i) => (
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                ...(i === activeIdx
                  ? { background: "linear-gradient(to right, var(--purple-primary), var(--purple-light))", color: "white", border: "none" }
                  : { background: "var(--surface)", color: "var(--surface-inverse-2)", border: "1px solid var(--line)" }),
              }}
            >
              {tab.label}
        ...
        <div className="bg-surface" style={{ padding: 32, borderRadius: 20, border: "1px solid rgba(128,21,232,0.08)", boxShadow: "0 18px 36px -24px rgba(64,12,140,0.2)" }}>
          <h3 className="font-bold" style={{ color: "var(--ink-heading)", fontSize: 22, marginBottom: 10 }}>{active.label}</h3>
          <p style={{ color: "var(--ink-soft)", fontSize: 15, lineHeight: "24px", marginBottom: 22 }}>{active.description}</p>
          <p className="font-bold" style={{ color: "var(--brand)", fontSize: 13, letterSpacing: "0.14em", textTransform: "uppercase", marginBottom: 12 }}>Key Features</p>
          <ul className="grid grid-cols-1 sm:grid-cols-2" style={{ gap: 10 }}>
            {(active.features || []).map((f: { emoji: string; text: string }, fi: number) => (
              <li key={f.text || fi} className="flex items-start" style={{ gap: 10 }}>
                ...
                <span style={{ color: "var(--ink-soft)", fontSize: 14, lineHeight: "22px" }}>{f.text}</span>
              </li>
            ))}
          </ul>
        </div>
    ...
    <section className="bg-surface px-4" style={{ paddingTop: 80, paddingBottom: 80 }}>
      <div className="mx-auto" style={{ maxWidth: 1100 }}>
        <h2 className="text-center font-bold" style={{ color: "var(--ink-heading)", fontSize: 36, lineHeight: "44px", marginBottom: 40 }}>
          Proven <span style={{ color: "var(--brand)" }}>ClickUp</span> Results
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4" style={{ gap: 20 }}>
          {stats.map((s, i) => (
            <div key={s.value || i} className="text-center" style={{ padding: 28, borderRadius: 18, background: "linear-gradient(180deg, var(--surface-tint) 0%, var(--line-tint) 100%)", border: "1px solid rgba(128,21,232,0.1)", boxShadow: "0 12px 28px -22px rgba(64,12,140,0.18)", display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
              <span className="flex items-center justify-center" style={{ width: 52, height: 52, borderRadius: 999, background: "var(--surface)", fontSize: 22, boxShadow: "0 4px 14px -8px rgba(64,12,140,0.25)" }}>{s.emoji}</span>
              <p className="font-bold" style={{ color: "var(--brand)", fontSize: 22, lineHeight: 1.1 }}>{s.value}</p>
              <p style={{ color: "var(--ink-soft)", fontSize: 13, lineHeight: "20px", maxWidth: 220 }}>{s.body}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <section className="bg-surface px-4" style={{ paddingTop: 80, paddingBottom: 80 }}>
      <div className="mx-auto" style={{ maxWidth: 1100 }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: 20 }}>
          {features.map((f, i) => (
            <div key={f.number || i} className="bg-surface" style={{ padding: 24, borderRadius: 16, border: "1px solid var(--line-tint)", display: "flex", gap: 16, alignItems: "flex-start" }}>
              <span className="flex items-center justify-center font-bold" style={{ width: 40, height: 40, borderRadius: 10, background: "linear-gradient(135deg, var(--purple-primary) 0%, var(--purple-light) 100%)", color: "white", fontSize: 13, flexShrink: 0 }}>
                {f.number}
              </span>
              <div>
                <p className="font-bold" style={{ color: "var(--ink-heading)", fontSize: 16, marginBottom: 6 }}>{f.title}</p>
                <p style={{ color: "var(--ink-soft)", fontSize: 13, lineHeight: "20px" }}>{f.body}</p>
              </div>
            </div>
          ))}
        </div>
